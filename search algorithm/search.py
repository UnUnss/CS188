            ...
            if state not in visited:
                for child_state, child_action, child_cost in problem.getSuccessors(state):
                    if child_state not in visited:
                        node = [child_state, action + [child_action],cost + child_cost]
                        frontier.push(node)
                visited.add(state)
        ...
        if problem.isGoalState(state) is True:
            ...
            if state not in visited:
                for child_state, child_action, child_cost in problem.getSuccessors(state):
                    if child_state not in visited:
                        node = [child_state, action + [child_action], cost + child_cost]
                        frontier.push(node,cost+child_cost)
                visited.add(state)
    ...
    while frontier.isEmpty() is not True:
        ...
        if state not in visited:
            for child_state, child_action, child_cost in problem.getSuccessors(state):
                if child_state not in visited:
                    node = [child_state, action + [child_action], cost + child_cost]
                    frontier.push(node,cost+child_cost+heuristic(child_state, problem))
            visited.add(state)
        else:
            continue
